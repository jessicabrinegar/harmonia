...
export class Note {
  private constructor(name: NoteName) {
    this.name = name;
  }
  ...
  static create(name: NoteName): Result<Note, Error> {
    if (!Object.values(NoteName).includes(name)) {
      return err(new Error(`Invalid note name: ${name}`));
    }
    return ok(new Note(name));
  }
  ...
  transpose(interval: Interval): Result<Note, Error> {
    return interval.semitones.andThen((semitones) => {
      const newPitch = (this.pitch + semitones) % 12;
      ...
      return Note.getNoteName(newNote, newPitch).map((name) => new Note(name));
    });
  }

  static getNoteName(letter: string, pitch: number): Result<NoteName, Error> {
    ...
    if (!matchingName) {
      return err(
        new Error(`No note name found for letter ${letter} and pitch ${pitch}`),
      );
    }
    return ok(matchingName);
  }
}
